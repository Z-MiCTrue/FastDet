...
class TensorDataset:
    def __init__(self, path: str, aug=False):
        self.aug = aug
        self.path = path
        self.data_list = []
            ...
            for line in f.readlines():
                ...
                if os.path.exists(data_path):
                    ...
                    if img_type not in self.img_formats:
                        raise Exception(f'img type error: {img_type}')
                    else:
                        self.data_list.append(data_path)
                else:
                    raise Exception(f'data_path: {data_path} is not exist')
        ...
        if os.path.exists(label_path):
            ...
            with open(label_path, 'r') as f:
                for line in f.readlines():
                    ...
                    label.append(['0'] + bbox_info)
            label = np.array(label, dtype=np.float32)
            if label.shape[0]:
                assert label.shape[1] == 6, f'need 5 label columns: {label_path}'
        else:
            raise Exception(f'label_path: {label_path} is not exist')
        # 是否进行数据增强
        if self.aug:
...
if __name__ == '__main__':
    data = TensorDataset('data/Darknet_Yolo/train.txt')
